...
- Salaah tab: `Salaah`, `Begins`, `Jamaat` (also accepts aliases: `Salah`/`Name`, `Athaan`/`Adhan`/`Start`, `Iqaamah`/`Iqamah`/`Congregation`). To change times, prepare a new tab in the sheet, publish it as CSV, and update the `salaah` URL in `_config.yml`
